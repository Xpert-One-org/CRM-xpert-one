  ...
  (
    ...
    const CreatableItem = () => {
      ...
      if (
        isOptionsExist(options, [
          { value: inputValue, label: inputValue, json_key: inputValue },
        ]) ||
      ...
      const Item = (
        <CommandItem
          ...
          onSelect={(value: string) => {
            ...
            const newOptions = [
              ...selected,
              { value, label: value, json_key: inputValue },
            ];
            setSelected(newOptions);
            onChange?.(newOptions);
          }}
    ...
    const EmptyItem = React.useCallback(() => {
      ...
      if (onSearch && !creatable && Object.keys(options).length === 0) {
        return (
          <CommandItem value="-" disabled className="">
            {emptyIndicator}
          </CommandItem>
        );
      }

      return <CommandEmpty className="bg-white">{emptyIndicator}</CommandEmpty>;
        ...
        >
          <div className="relative flex w-full flex-wrap items-center gap-1 pr-2">
            ...
            <CommandPrimitive.Input
              ...
              className={cn(
                'placeholder:text-muted-foreground flex-1 bg-transparent pr-6 outline-none',
                {
