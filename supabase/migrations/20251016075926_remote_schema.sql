...
CREATE TYPE "public"."base_tarifaire_type" AS ENUM (
    'TJM',
    'Prix Mensuel',
    'Brut Mensuel'
);
...
ALTER TYPE "public"."base_tarifaire_type" OWNER TO "postgres";
...
ALTER TYPE "public"."xpert_status_type" OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."handle_storage_notification"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$declare
  parsed_info record;
  mission_record record;
  xpert_profile record;
begin
  -- Extraire les informations du chemin
  select * from parse_file_path(NEW.name) into parsed_info;
  
  -- Récupérer les informations de la mission
  select * from mission 
  where mission_number = parsed_info.mission_number 
  limit 1 into mission_record;
  
  -- Récupérer le profil de l'xpert
  select * from profile 
  where generated_id = parsed_info.xpert_id 
  limit 1 into xpert_profile;
  

  -- Si le fichier est ajouté par le référent (basé sur le chemin ou les métadonnées)
  if NEW.owner = xpert_profile.id then
    -- Vérifier que l'xpert existe toujours avant d'envoyer la notification
    INSERT INTO notification (
        user_id,
        link,
        message,
        ...
        xpert_profile.affected_referent_id,
         'mission/fiche/M-' || parsed_info.mission_number,
        'Un document a été ajouté à la mission M ' || parsed_info.mission_number,
        'Document mission',
        'info',
        false,
        null,
    ...
    -- Si l'xpert existe, envoyer une notification
    if exists (select 1 from profile where id = xpert_profile.id) then
      INSERT INTO notification (
        user_id,
        link,
        message,
        ...
        xpert_profile.id,
         'mission/fiche/M-' || parsed_info.mission_number,
        'Un document a été ajouté par votre référent à la mission M ' || parsed_info.mission_number,
        'Document mission',
        'info',
        false,
        null,
...
ALTER FUNCTION "public"."handle_storage_notification"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."insert_mission_finance"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."insert_mission_finance"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."parse_file_path"("file_path" "text") RETURNS TABLE("mission_number" "text", "xpert_id" "text", "document_type" "text")
    LANGUAGE "plpgsql"
    AS $$begin
  return query
 select 
   split_part(split_part(file_path, '/', 1), ' ', 2) as mission_number,
    split_part(file_path, '/', 2) as xpert_id,
    split_part(file_path, '/', 5) as document_type;
end;$$;
...
CREATE TABLE IF NOT EXISTS "public"."mission_finance" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "mission_id" bigint NOT NULL,
    "xpert_status" "public"."xpert_status_type",
    "base_tarifaire" "public"."base_tarifaire_type",
...
ALTER TABLE "public"."mission_finance" OWNER TO "postgres";


ALTER TABLE "public"."mission_finance" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."mission_finance_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE "public"."profile_bans" OWNER TO "postgres";
...
CREATE OR REPLACE VIEW "public"."unique_posts" WITH ("security_invoker"='on') AS
...
CREATE OR REPLACE VIEW "public"."unique_posts_with_referents" WITH ("security_invoker"='on') AS
 SELECT "u"."post",
...
CREATE OR REPLACE VIEW "public"."view_kbis" WITH ("security_invoker"='on') AS
 SELECT "subquery"."generated_id" AS "id_utilisateur",
    ((('https://wxjnrjakjwjhvsiwhelt.supabase.co/storage/v1/object/public/profile_files/'::"text" || "subquery"."generated_id") || '/kbis/kbis_'::"text") || "subquery"."timestamp") AS "kbi_url",
    "subquery"."timestamp" AS "upload_timestamp"
   FROM ( SELECT DISTINCT ON ("profile"."generated_id") "profile"."generated_id",
            "substring"("objects"."name", '^.*kbis_(\d+)'::"text") AS "timestamp"
           FROM (("storage"."objects"
             JOIN "public"."profile" ON (("objects"."owner" = "profile"."id")))
             JOIN "auth"."users" ON (("objects"."owner" = "users"."id")))
          WHERE ("objects"."name" ~~ '%/kbis/kbis_%'::"text")
          ORDER BY "profile"."generated_id", ("substring"("objects"."name", '^.*kbis_(\d+)'::"text"))::bigint DESC) "subquery"
  ORDER BY "subquery"."timestamp" DESC;


ALTER TABLE "public"."view_kbis" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."mission_finance"
    ADD CONSTRAINT "mission_finance_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."profile_bans"
    ADD CONSTRAINT "profile_bans_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "idx_profile_bans_banned_by" ON "public"."profile_bans" USING "btree" ("banned_by");



CREATE INDEX "idx_profile_bans_is_active" ON "public"."profile_bans" USING "btree" ("is_active");



CREATE INDEX "idx_profile_bans_profile_id" ON "public"."profile_bans" USING "btree" ("profile_id");
...
CREATE OR REPLACE TRIGGER "brevo_new_echo_chat" AFTER INSERT ON "public"."chat" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://wxjnrjakjwjhvsiwhelt.supabase.co/functions/v1/brevo-management', 'POST', '{"Content-type":"application/json"}', '{"type":"new_echo_chat"}', '5000');



CREATE OR REPLACE TRIGGER "brevo_new_message" AFTER INSERT ON "public"."message" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://wxjnrjakjwjhvsiwhelt.supabase.co/functions/v1/brevo-management', 'POST', '{"Content-type":"application/json"}', '{"type":"new_message"}', '5000');



CREATE OR REPLACE TRIGGER "brevo_new_mission_application" AFTER INSERT ON "public"."mission_application" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://wxjnrjakjwjhvsiwhelt.supabase.co/functions/v1/brevo-management', 'POST', '{"Content-type":"application/json"}', '{"type":"new_mission_application"}', '5000');



CREATE OR REPLACE TRIGGER "brevo_new_user_deleted" AFTER INSERT ON "public"."profile_deleted" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://wxjnrjakjwjhvsiwhelt.supabase.co/functions/v1/brevo-management', 'POST', '{"Content-type":"application/json"}', '{"type":"new_user_deleted"}', '5000');



CREATE OR REPLACE TRIGGER "brevo_new_xpert_to_xpert_chat" AFTER INSERT ON "public"."chat" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://wxjnrjakjwjhvsiwhelt.supabase.co/functions/v1/brevo-management', 'POST', '{"Content-type":"application/json"}', '{"type":"new_xpert_to_xpert_chat"}', '5000');
...
ALTER TABLE ONLY "public"."mission_finance"
    ADD CONSTRAINT "mission_finance_mission_fkey" FOREIGN KEY ("mission_id") REFERENCES "public"."mission"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."profile_bans"
    ADD CONSTRAINT "profile_bans_banned_by_fkey" FOREIGN KEY ("banned_by") REFERENCES "public"."profile"("id");



ALTER TABLE ONLY "public"."profile_bans"
    ADD CONSTRAINT "profile_bans_profile_id_fkey" FOREIGN KEY ("profile_id") REFERENCES "public"."profile"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."profile_bans"
    ADD CONSTRAINT "profile_bans_unbanned_by_fkey" FOREIGN KEY ("unbanned_by") REFERENCES "public"."profile"("id");
...
ALTER TABLE ONLY "public"."xpert_notes"
    ADD CONSTRAINT "xpert_notes_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."profile"("id") ON UPDATE CASCADE ON DELETE CASCADE;
...
CREATE POLICY "Enable CRUD for all users" ON "public"."chat" USING (true);



CREATE POLICY "Enable CRUD for all users" ON "public"."contact_xpert_demands" USING (true);



CREATE POLICY "Enable CRUD for all users" ON "public"."mission" USING (true);



CREATE POLICY "Enable CRUD for all users" ON "public"."mission_canceled" USING (true);



CREATE POLICY "Enable CRUD for all users" ON "public"."mission_checkpoints" USING (true);



CREATE POLICY "Enable CRUD for all users" ON "public"."profile_deleted" USING (true);



CREATE POLICY "Enable CRUD for all users" ON "public"."task_history" USING (true);



CREATE POLICY "Enable CRUD for all users" ON "public"."user_alerts" USING (true);
...
ALTER TABLE "public"."mission_canceled" ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "mission_finance_policy" ON "public"."mission_finance" USING ((EXISTS ( SELECT 1
   FROM "public"."profile" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."profile_roles", 'project_manager'::"public"."profile_roles", 'intern'::"public"."profile_roles", 'hr'::"public"."profile_roles", 'adv'::"public"."profile_roles"]))))));
...
CREATE POLICY "profile_ban_fields_policy" ON "public"."profile" FOR UPDATE TO "authenticated" USING (((EXISTS ( SELECT 1
   FROM "public"."profile" "profile_1"
  WHERE (("profile_1"."id" = "auth"."uid"()) AND ("profile_1"."role" = 'admin'::"public"."profile_roles")))) OR ("id" = "auth"."uid"()))) WITH CHECK (((EXISTS ( SELECT 1
   FROM "public"."profile" "profile_1"
  WHERE (("profile_1"."id" = "auth"."uid"()) AND ("profile_1"."role" = 'admin'::"public"."profile_roles")))) OR (("id" = "auth"."uid"()) AND ("is_banned_from_community" = ( SELECT "profile_1"."is_banned_from_community"
   FROM "public"."profile" "profile_1"
  WHERE ("profile_1"."id" = "auth"."uid"()))) AND ((("community_banning_explanations" IS NULL) AND (( SELECT "profile_1"."community_banning_explanations"
   FROM "public"."profile" "profile_1"
  WHERE ("profile_1"."id" = "auth"."uid"())) IS NULL)) OR ("community_banning_explanations" = ( SELECT "profile_1"."community_banning_explanations"
   FROM "public"."profile" "profile_1"
...
CREATE POLICY "profile_bans_delete_policy" ON "public"."profile_bans" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profile"
  WHERE (("profile"."id" = "auth"."uid"()) AND ("profile"."role" = 'admin'::"public"."profile_roles")))));



CREATE POLICY "profile_bans_insert_policy" ON "public"."profile_bans" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profile"
  WHERE (("profile"."id" = "auth"."uid"()) AND ("profile"."role" = 'admin'::"public"."profile_roles")))));



CREATE POLICY "profile_bans_select_policy" ON "public"."profile_bans" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profile"
  WHERE (("profile"."id" = "auth"."uid"()) AND (("profile"."role" = 'admin'::"public"."profile_roles") OR ("profile"."id" = "profile_bans"."profile_id"))))));



CREATE POLICY "profile_bans_update_policy" ON "public"."profile_bans" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profile"
  WHERE (("profile"."id" = "auth"."uid"()) AND ("profile"."role" = 'admin'::"public"."profile_roles")))));



ALTER TABLE "public"."profile_deleted" ENABLE ROW LEVEL SECURITY;
...
GRANT ALL ON FUNCTION "public"."handle_storage_notification"() TO "anon";
GRANT ALL ON FUNCTION "public"."handle_storage_notification"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."handle_storage_notification"() TO "service_role";



GRANT ALL ON FUNCTION "public"."insert_mission_finance"() TO "anon";
GRANT ALL ON FUNCTION "public"."insert_mission_finance"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."insert_mission_finance"() TO "service_role";
...
GRANT ALL ON TABLE "public"."mission_finance" TO "anon";
GRANT ALL ON TABLE "public"."mission_finance" TO "authenticated";
GRANT ALL ON TABLE "public"."mission_finance" TO "service_role";



GRANT ALL ON SEQUENCE "public"."mission_finance_id_seq" TO "anon";
GRANT ALL ON SEQUENCE "public"."mission_finance_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."mission_finance_id_seq" TO "service_role";
...
GRANT ALL ON TABLE "public"."profile_bans" TO "anon";
GRANT ALL ON TABLE "public"."profile_bans" TO "authenticated";
GRANT ALL ON TABLE "public"."profile_bans" TO "service_role";
...
GRANT ALL ON TABLE "public"."view_kbis" TO "anon";
GRANT ALL ON TABLE "public"."view_kbis" TO "authenticated";
GRANT ALL ON TABLE "public"."view_kbis" TO "service_role";
