...
type MissionNotesProps = {
  missionId: number;
};

export function MissionNotes({ missionId }: MissionNotesProps) {
  const [notes, setNotes] = useState<DBMissionNote[]>([]);
  ...
  const fetchNotes = async () => {
    const { data, error } = await getMissionNotes(missionId);
    if (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    const { error } = await createMissionNote(missionId, newNote);
    setIsLoading(false);
  ...
  const handleUpdate = async (noteId: number, content: string) => {
    const { error } = await updateMissionNote(noteId, content);
    if (error) {
  ...
  const handleDelete = async (noteId: number) => {
    const { error } = await deleteMissionNote(noteId);
    if (error) {
