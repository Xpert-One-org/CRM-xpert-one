// components/FournisseurNotes.tsx
'use client';
...
import {
  ...
  deleteSupplierNote,
  updateSupplierNote,
} from '../../../../functions/fournisseurs-notes';
...
type FournisseurNotesProps = {
  fournisseurId: string;
};

export function FournisseurNotes({ fournisseurId }: FournisseurNotesProps) {
  const [notes, setNotes] = useState<DBFournisseurNote[]>([]);
  ...
  const fetchNotes = async () => {
    const { data, error } = await getSupplierNotes(fournisseurId);
    if (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    const { error } = await createSupplierNote(fournisseurId, newNote);
    setIsLoading(false);
  ...
  const handleUpdate = async (noteId: number, content: string) => {
    const { error } = await updateSupplierNote(noteId, content);
    if (error) {
  ...
  const handleDelete = async (noteId: number) => {
    const { error } = await deleteSupplierNote(noteId);
    if (error) {
